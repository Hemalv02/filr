...
export function ToonConfirmationPage({ 
  ...
  const renderField = (label: string, field: keyof ExtractedData, isImported: boolean) => {
    ...
    return (
      <div key={field} className="space-y-2">
        ...
        <Input
          ...
          className={isImported ? "border-blue-300 bg-blue-50/50" : ""}
        />
      </div>
    );
  };
              ...
              <Card key={group.title}>
                ...
                <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {group.fields.map(({ key, label }) => {
                    ...
                    return renderField(label, key, isFieldImported(key));
                  })}
                </CardContent>
              </Card>
            );
